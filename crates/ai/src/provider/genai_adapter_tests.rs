...
async fn test_tool_calls() {
    ...
    let request = ChatRequest {
        messages: vec![ChatMessage::User(vec![ContentBlock::Text("What's the weather in SF?".into())])],
        tools,
        options: Default::default(),
    };
    ...
    assert!(input.get("location").is_some(), "Tool call should have location parameter");
}
...
async fn test_streaming() {
    ...
    assert!(matches!(events[0], StreamEvent::Start), "First event should be Start");
    assert!(events.iter().filter(|e| matches!(e, StreamEvent::TextChunk(_))).count() >= 3, "Should have multiple text chunks");
    assert!(matches!(events.last().unwrap(), StreamEvent::End { .. }), "Last event should be End");
}
